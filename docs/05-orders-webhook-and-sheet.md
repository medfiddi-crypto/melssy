...
5. Worker sends signed event to Apps Script with bounded exponential retry.
...
Headers are in [orders-sheet-template.csv](orders-sheet-template.csv). One row per order; `items_json` carries line items. Raw/normalized phone is sensitive. Dates are ISO 8601 UTC and money is decimal MAD, never formatted display text. Repeated `event_id` must be idempotent; newest `occurred_at` wins.
