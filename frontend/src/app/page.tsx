...
export default function Home() {
  ...
  const placeOrder = async () => {
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8000"}/v1/orders`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, phone, idempotency_key: safeUUID(), items: Object.entries(quantities).map(([product_id, quantity]) => ({ product_id, quantity })) }) });
  ...
  return <main>
    <header className="sticky top-0 z-20 flex items-center justify-between border-b border-[var(--line)] bg-[var(--background)]/95 px-5 py-3 backdrop-blur"><button ref={menuButtonRef} aria-label="Ouvrir le menu" aria-expanded={menu} aria-controls="site-menu" onClick={() => setMenu(true)}>Menu</button><a href="#rituel" className="text-center"><span className="wordmark text-2xl">MELSSY</span><span className="block text-[8px] tracking-[.18em]">BEAUTY SLEEP RITUAL</span></a><button aria-label="Ouvrir le panier" onClick={() => setDrawer(true)}>Sac ({cart.length})</button></header>
