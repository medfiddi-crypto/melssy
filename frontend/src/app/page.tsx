...
export default function Home() {
  ...
  return <main>
    ...
    <section className="grid md:grid-cols-2"><div className="px-6 py-16 md:px-16"><p className="text-sm text-[var(--rose)]">{landing.satinComparison.eyebrow}</p><h2 className="display mt-3 text-4xl leading-none md:text-5xl">{landing.satinComparison.headline}</h2><p className="mt-5 text-lg leading-7">{landing.satinComparison.subheadline}</p><div className="mt-8 space-y-6">{landing.satinComparison.blocks.map((block) => <div key={block.lead}><p className="font-medium leading-6">{block.lead}</p><p className="mt-2 leading-7 text-black/75">{block.text}</p></div>)}</div><p className="mt-8 border-t border-[var(--line)] pt-5 text-sm leading-6 text-black/70">{landing.satinComparison.closing}</p></div><SectionImage src={media.satinComparison.src} alt={landing.satinComparison.imageAlt} sizes="(max-width: 768px) calc(100vw - 48px), 50vw" /></section>
