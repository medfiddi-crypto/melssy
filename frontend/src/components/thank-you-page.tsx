...
export function ThankYouPage({ orderNumber, initialOrder = null }: { orderNumber: string; initialOrder?: ConfirmedOrder | null }) {
  ...
  return <main className="min-h-dvh bg-[var(--background)] px-5 py-8 pb-[calc(2rem+env(safe-area-inset-bottom))] sm:px-6">
    ...
    <section className="mx-auto max-w-xl pt-12">
      ...
      <section className="mt-10"><p className="eyebrow text-[var(--rose)]">PROCHAINES ÉTAPES</p><div className="mt-4 divide-y divide-[var(--line)] border-y border-[var(--line)]"><Step icon={Phone}>Nous vous appellerons depuis le {confirmationPhone} sous {confirmationWindow}. Enregistrez ce numéro dans vos contacts.</Step><Step icon={Truck}>Votre coffret est expédié sous {dispatchWindow}, puis livré en {deliveryWindow}, partout au Maroc.</Step><Step icon={PackageCheck}>Vous réglez {total} en espèces au livreur. Vous ne payez rien à l&apos;avance.</Step></div></section>

      <section className="mt-10 border border-[var(--line)] bg-white p-5"><p className="eyebrow text-[var(--rose)]">Votre commande</p><div className="mt-5 space-y-3 text-sm">{order.items.map((item) => <div key={`${item.product_id}-${item.unit_price}`} className="flex items-start justify-between gap-4"><span>{item.product_id === "beauty-night-ritual" ? "Coffret Beauty Night Ritual" : item.product_name}</span><span className="shrink-0">{formatPrice(Number(item.unit_price) * item.quantity)}</span></div>)}<div className="flex items-start justify-between gap-4"><span>{storefront.freeGiftName()}</span><span className="shrink-0 text-[var(--green)]">Offert</span></div><div className="flex items-start justify-between gap-4"><span>Livraison</span><span className="shrink-0"><s className="text-black/50">{shippingFee}</s><span className="ml-2 text-[var(--green)]">Offerte</span></span></div></div><div className="mt-5 flex items-end justify-between gap-4 border-t border-[var(--line)] pt-5"><span className="text-sm font-medium">À régler à la livraison</span><strong className="display text-3xl font-medium">{total}</strong></div></section>
