...
export function ThankYouPage({ orderNumber, initialOrder = null }: { orderNumber: string; initialOrder?: ConfirmedOrder | null }) {
  ...
  return <main className="min-h-dvh bg-[var(--background)] px-5 py-8 pb-[calc(2rem+env(safe-area-inset-bottom))] sm:px-6">
    ...
    <section className="mx-auto max-w-xl pt-12">
      ...
      <section className="mt-10"><p className="eyebrow text-[var(--rose)]">PROCHAINES ÉTAPES</p><div className="mt-4 divide-y divide-[var(--line)] border-y border-[var(--line)]">{confirmationWindow && <Step icon={Phone}>Nous vous appelons sous {confirmationWindow} pour confirmer votre commande.</Step>}{dispatchWindow && deliveryWindow && <Step icon={Truck}>Votre coffret est expédié sous {dispatchWindow}, puis livré en {deliveryWindow}, partout au Maroc.</Step>}<Step icon={PackageCheck}>Vous réglez {total} en espèces au livreur. Vous ne payez rien à l&apos;avance.</Step></div></section>
